...
| `formatvalues.go` | `ComputeFormatValues(ver SemanticVersion, cfg FormatConfig) map[string]string` — pure function computing all 25+ output variable strings including Assembly variables (`AssemblySemVer`, `AssemblySemFileVer`, `AssemblyInformationalVersion`) and NuGet variables (`NuGetVersionV2`, `NuGetPreReleaseTagV2`). Output-only — no file writes. `FormatConfig` holds padding, commit-date-format, tag-pre-release-weight |
