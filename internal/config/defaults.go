...
func CreateDefaultConfiguration() *Config {
	return &Config{
		...
		MajorVersionBumpMessage:          stringPtr(`(\+semver:\s?(breaking|major)|bump major:)`),
		MinorVersionBumpMessage:          stringPtr(`(\+semver:\s?(feature|minor)|bump minor:)`),
		PatchVersionBumpMessage:          stringPtr(`(\+semver:\s?(fix|patch)|bump patch:)`),
		NoBumpMessage:                    stringPtr(`(\+semver:\s?(none|skip)|bump (none|skip):)`),
