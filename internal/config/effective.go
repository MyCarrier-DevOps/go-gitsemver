...
func NewEffectiveConfiguration(cfg *Config, branch *BranchConfig) EffectiveConfiguration {
	ec := EffectiveConfiguration{
		...
		MajorVersionBumpMessage:          derefString(cfg.MajorVersionBumpMessage, `(\+semver:\s?(breaking|major)|bump major:)`),
		MinorVersionBumpMessage:          derefString(cfg.MinorVersionBumpMessage, `(\+semver:\s?(feature|minor)|bump minor:)`),
		PatchVersionBumpMessage:          derefString(cfg.PatchVersionBumpMessage, `(\+semver:\s?(fix|patch)|bump patch:)`),
		NoBumpMessage:                    derefString(cfg.NoBumpMessage, `(\+semver:\s?(none|skip)|bump (none|skip):)`),
